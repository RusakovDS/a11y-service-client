...
export class ReportsService {
  ...
  getAllByProjectId(projectId: number) {
    return this.apiService.get(`reports/project/${projectId}`).pipe(
      map((receivedData) => {
  ...
  getOneById(reportId) {
    return this.apiService.get(`reports/${reportId}`).pipe(
      map((receivedData) => {
  ...
  create(id: ID) {
    return this.apiService.post("reports", { id }).pipe(
      map((result) => {
        if (result) {
          this.reportsStore.add({ ...result, status: "new" });
          this.projectsStore.update(
            result.projectId,
            arrayAdd<Project>("report", result.id)
          );
        }
      })
    );
