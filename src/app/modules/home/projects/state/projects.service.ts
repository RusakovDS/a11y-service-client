...
export class ProjectsService {
  ...
  checkCache() {
    return this.projectsQuery.selectHasCache().pipe(
      switchMap((hasCache) => {
  ...
  create(formValues: Project) {
    return this.apiService.post("projects", formValues).pipe(
      map((newEntity) => {
        if (newEntity) {
  ...
  edit(formValues: Project, id: ID) {
    return this.apiService.put(`projects/${id}`, formValues).pipe(
      map((updEntity) => {
        if (updEntity) {
